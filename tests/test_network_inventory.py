    ...
    def __init__(self):
        ...
        for table, fields in TABLES.items():
            ...
            for field in fields.split():
                numeric = field in ("IsOnline", "IsActive", "KepwareSnapshotComplete", "Enabled", "TotalIPs", "OnlineCount")
...
def test_manual_revisions_preserved_latest_wins_over_scan_and_kepware(inventory):
    ...
    history = service.store.history(IP)["manual"]
...
def test_kepware_name_and_hostname_fallback(inventory):
    ...
    assert service.store.current(service.addresses)["rows"][0]["MachineName"] == "Mixer"
...
def test_removed_and_previously_online_devices_never_free(inventory):
    ...
    online_history = [r for r in service.store.history(END)["network"] if r["IsOnline"]]
    assert rows[2]["LastSeen"] == online_history[0]["ScanTime"]
    assert len(service.store.history(IP)["kepware"]) == 1
...
def test_no_scan_and_probe_error_are_not_free(inventory):
    service, _ = inventory
    assert {r["Status"] for r in service.store.current(service.addresses)["rows"]} == {"Not Verified"}
    service.probe.side_effect = lambda ip: scan_result(ip, ScanError="tool failed")
    service.scan("test")
    assert service.store.current(service.addresses)["rows"][1]["Status"] == "Not Verified"
...
def test_search_filters_sort_and_timestamps(inventory):
    ...
    run = service.scan("test")
    current = service.store.current(service.addresses)
    rows = current["rows"]
...
def test_multiple_kepware_identities_same_ip(inventory):
    ...
    service.scan("test")
    row = service.store.current(service.addresses)["rows"][0]
...
def test_inactive_manual_evidence_still_reserves_ip(inventory):
    service, _ = inventory
    service.store.manual(dict(IPAddress=END, MachineName="Old reservation", Description="", Location="",
                              Remark="", UpdatedAt=now(), UpdatedBy="test", IsActive=False))
    service.scan("test")
    row = service.store.current(service.addresses)["rows"][2]
...
def test_api_current_history_manual_and_scan_use_configured_service(inventory):
    ...
    with patch.multiple(OpcTagManager.network_inventory, store=service.store, client=service.client,
                        start=IP, end=END, probe=service.probe):
