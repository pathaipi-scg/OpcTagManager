...
class OpcTagManagerAppTests(unittest.TestCase):
    ...
    def test_home_and_static_assets_render_without_external_services(self, _get_conn):
        ...
        self.assertEqual(html.count('class="knowledge-attach-button"'), 4)
        self.assertEqual(html.count('class="knowledge-image-input hidden"'), 4)
        self.assertEqual(html.count('paste a screenshot with Ctrl+V'), 4)
        self.assertEqual(html.count('id="knowledge-description"'), 1)
        self.assertIn('How to Check / Troubleshooting', html)
        self.assertNotIn('<label for="knowledge-possible-cause">', html)
        self.assertIn('id="knowledge-possible-cause" class="hidden"', html)
        ...
        self.assertLess(html.index('data-alarm-filter="all"'), html.index('data-alarm-filter="list"'))
        self.assertLess(html.index('data-alarm-filter="list"'), html.index('data-alarm-filter="alarm"'))
        self.assertIn('id="alarm-tag-tree"', html)
        ...
        self.assertIn('<strong>Alarm List</strong>', html)
        self.assertNotIn('<strong>Current Alarm Mapping</strong>', html)
        self.assertNotIn('id="runtime-mp3-panel"', html)
        self.assertIn('id="alarm-center-splitter"', html)
        self.assertNotIn('id="alarm-horizontal-splitter"', html)
        ...
        self.assertIn('class="alarm-configuration-pane"', html)
        self.assertIn('<h2>Alarm Configuration</h2>', html)
        self.assertIn('class="tag-knowledge-pane"', html)
        self.assertIn('<h2>Tag Knowledge</h2>', html)
        ...
        navigation = html.split('<nav class="view-tabs"', 1)[1].split('</nav>', 1)[0]
        self.assertIn('id="theme-toggle"', navigation)
        self.assertEqual(html.count('id="theme-toggle"'), 1)
        self.assertNotIn('class="header-bar"', html)
        ...
        self.assertNotIn('<h2>Tag Configuration Tree</h2>', html)
        self.assertNotIn('<h1>OpcTagManager</h1>', html)
        self.assertNotIn('<h2>OPC Tag List</h2>', html)
        self.assertNotIn('<h2>OT Network Inventory</h2>', html)
        ...
        self.assertNotIn('<h2>Alarm Detail</h2>', html)
        self.assertIn('id="alarm-help-open-tag" class="hidden"', html)
        self.assertIn('Open in Tag Configuration', html)
        self.assertIn('<h2>Recent Alarm History</h2>', html)
        self.assertIn('<strong>Alarm Activity</strong>', html)
        self.assertNotIn('Select an object in the Tag Configuration Tree.', html)
        self.assertNotIn('Actual results returned by InfluxWriter.write().', html)
        self.assertNotIn('Diagnostics only. alarm_sound remains playback authority.', html)
        inventory_topbar = html.split('class="inventory-toolbar inventory-topbar"', 1)[1].split('</div>', 1)[0]
        for control in ('inventory-scan', 'inventory-refresh', 'inventory-free', 'inventory-range', 'inventory-freshness'):
            self.assertIn(f'id="{control}"', inventory_topbar)
        self.assertIn('id="alarm-help-activity-row"', html)
        self.assertLess(html.index('class="alarm-help-activity panel"'), html.index('id="alarm-help-main"'))
        self.assertIn('id="alarm-help-main" class="alarm-help-main history-collapsed"', html)
        self.assertIn('id="alarm-help-toggle-history"', html)
        self.assertIn('id="alarm-help-toggle-history" class="alarm-help-sidebar-toggle" aria-controls="alarm-help-history" aria-expanded="false"', html)
        self.assertIn('id="alarm-help-open-history" class="alarm-help-open-history"', html)
        self.assertNotIn('id="alarm-help-open-history" class="alarm-help-open-history hidden"', html)
        history_header = html[html.index('class="alarm-help-history-scroll"'):html.index('id="alarm-help-history-body"')]
        self.assertIn('<th>Time</th><th>Tag Name</th>', history_header)
        self.assertNotIn('<th>Kepware Path</th>', history_header)
        self.assertNotIn('<th>Priority</th>', history_header)
        self.assertNotIn('<th>State</th>', history_header)
        detail = html[html.index('class="alarm-help-detail panel"'):html.index('</article>', html.index('class="alarm-help-detail panel"'))]
        self.assertIn('<dt>Kepware Path</dt>', detail)
        self.assertIn('<dt>Priority</dt>', detail)
        self.assertIn('<dt>State</dt>', detail)
        self.assertIn('id="alarm-help-back-latest"', detail)
        self.assertIn('<summary>API Endpoints</summary>', html)
        history_sidebar = html[html.index('<aside id="alarm-help-history"'):html.index('</aside>', html.index('<aside id="alarm-help-history"'))]
        self.assertIn('<summary>API Endpoints</summary>', history_sidebar)
        self.assertIn('<summary>Raw JSON</summary>', history_sidebar)
        self.assertNotIn('<details class="alarm-help-commissioning panel" open', history_sidebar)
        self.assertNotIn('<details class="alarm-help-diagnostics panel" open', history_sidebar)
        self.assertLess(history_sidebar.index('id="alarm-help-history-body"'), history_sidebar.index('<summary>API Endpoints</summary>'))
        self.assertLess(history_sidebar.index('<summary>API Endpoints</summary>'), history_sidebar.index('<summary>Raw JSON</summary>'))
        alarm_detail = html[html.index('<article class="alarm-help-detail panel"'):html.index('</article>', html.index('<article class="alarm-help-detail panel"'))]
        self.assertNotIn('<summary>API Endpoints</summary>', alarm_detail)
        self.assertNotIn('<summary>Raw JSON</summary>', alarm_detail)
        ...
        self.assertIn('`${data.alarms.length} alarms`', javascript)
        ...
        self.assertIn('const visibleKnowledgeSections = ["how_to_check", "corrective_action", "safety_warning", "additional_notes"]', javascript)
        ...
        self.assertIn('alarmTopWorkspace.classList.toggle("hidden", isKepware || isOpcRuntime || isAlarmHelp || isInventory)', javascript)
        self.assertIn('data-view="network-inventory"', html)
        ...
        self.assertIn('alarmHelpDisplayedPath = alarm.kepware_path || null', javascript)
        self.assertIn('document.getElementById("alarm-help-open-tag").classList.toggle("hidden", !alarmHelpDisplayedPath)', javascript)
        self.assertIn('async function openTagByKepwarePath(kepwarePath, error)', javascript)
        self.assertIn('openTagByKepwarePath(alarmHelpDisplayedPath', javascript)
        self.assertIn('openTagByKepwarePath(alarmHelpActivityPath', javascript)
        self.assertIn('id="alarm-help-activity-open-tag" class="runtime-open-tag hidden"', html)
        self.assertIn('open.className = "runtime-open-tag"', javascript)
        self.assertIn('kepwarePath: entry.path || null', javascript)
        self.assertGreaterEqual(javascript.count('openTagByKepwarePath('), 4)
        self.assertIn('await loadKepwareChannels()', javascript)
        self.assertIn('await ensureKepwareChildren(channel)', javascript)
        self.assertIn('await ensureKepwareChildren(device)', javascript)
        self.assertIn('await ensureKepwareChildren(group)', javascript)
        self.assertIn('document.querySelector(\'.view-tab[data-view="runtime"]\').click()', javascript)
        self.assertIn('selectKepwareObject(target, target.kepwareNode)', javascript)
        self.assertIn('revealNavigationItem(target)', javascript)
        self.assertIn('item.scrollIntoView({ block: "nearest" })', javascript)
        self.assertIn('Tag is no longer available in the current Kepware configuration.', javascript)
        self.assertIn('button.kepwareNode = node', javascript)
        self.assertIn('async function ensureKepwareChildren(button)', javascript)
        ...
        self.assertIn('[alarm.activated_at, alarm.tag_name].forEach', javascript)
        self.assertIn('loadAlarmHelpHistoryDetail(alarm.history_id)', javascript)
        self.assertIn('row.classList.toggle("selected-history"', javascript)
        self.assertIn('setAlarmHelpHistoryOpen(false)', javascript)
        self.assertIn('setAlarmHelpHistoryOpen(true)', javascript)
        self.assertIn('main.classList.toggle("history-collapsed", !isOpen)', javascript)
        stylesheet = Path("static/app.css").read_text(encoding="utf-8")
        self.assertIn('grid-template-columns: minmax(250px, 28%) minmax(0, 1fr)', stylesheet)
        self.assertIn('.alarm-help-main.history-collapsed', stylesheet)
        ...
        self.assertIn('syncAlarmNavigationSelection();', javascript)
        self.assertIn('item.classList.toggle(highlight, item === target)', javascript)
        ...
        self.assertNotIn('.alarm-horizontal-splitter', stylesheet)
        self.assertNotIn('cursor: row-resize', stylesheet)
        self.assertIn('#runtime-tree-view { display: flex; flex-direction: column; height: 100%; min-height: 0; }', stylesheet)
        ...
        self.assertIn('.knowledge-section-how-to-check textarea, .knowledge-section-corrective-action textarea', stylesheet)
        self.assertIn('grid-template-columns: minmax(0, 2fr) minmax(0, 3fr)', stylesheet)
        self.assertIn('.alarm-configuration-pane, .tag-knowledge-pane', stylesheet)
