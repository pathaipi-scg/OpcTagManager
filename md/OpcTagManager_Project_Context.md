...
Phases 4.8 and 4.9 are complete and approved. Phase 4.10, Canonical Integration Contracts, is implemented and awaits review. It normalizes `canonical_revision` from the active immutable version and SHA-256, adds generic read-only canonical preflight and bounded TagMaster search, and adds a gated multipart Factory-KM handoff for canonical Manual, Drawing, Quotation, and General Document Resources. Factory-KM command execution and relationship mutation remain unimplemented and unauthorized.

The future executor contract is now explicit: retain the canonical ID and revision selected during human review; immediately before a mutation, read current state through `GET /api/canonical/{canonical_id}`; mark the command `CONFLICT` if the revision differs; otherwise perform only the separately authorized mutation. Document canonicalization uses `POST /api/integration/resources` with document bytes, declared SHA-256, logical Factory-KM provenance, and no client filesystem locator. Exact SHA retries resolve to the existing canonical Resource; similar names with different content remain an explicit human decision.
